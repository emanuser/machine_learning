...
---
#### In this project, the goal was  to use data from accelerometers on the belt, forearm, arm, and dumbell of 6 participants to predict how well they preformed the exercises. They were asked to perform barbell lifts correctly and incorrectly in 5 different ways. More information is available from the website here: http://groupware.les.inf.puc-rio.br/har (see the section on the Weight Lifting Exercise Dataset).
...
### Load and proses data
#### Manualy removed unwanted columns, only keped rawdata
...
### Use caret to buidl  traning, testing, and cross validation set
```{r, echo=TRUE}
set.seed(245)
...
## Feture extraction and formula creation
### Split traning set iin to subsets by class
### The evaluator function was adaped from an exaple in the FSelector packeg
### The function is used in cconjuction with best.first.search function to create a smaller feture set which best represtnts the data  
#### 
...
evaluator <- function(subset) {
  ...
  k <- 5
  splits <- runif(nrow(testing))
  results = sapply(1:k, function(i) {
    test.idx <- (splits >= (i - 1) / k) & (splits < i / k)
    train.idx <- !test.idx
    test <- testing[test.idx, , drop=FALSE]
    train <- testing[train.idx, , drop=FALSE]
...
## Modle selection
### Used random forest and gradiant boosting on each of the five formulas for a total of ten individuyal modles
...
## Model preformance on testing set

### Each model has some short comings do to the small amount of fetures
### Uesd a support vector machine to run the individual model predictions in agraget
```{r, echo=TRUE, cache=TRUE}
...
## Model preformance on validation set

### This is done to test out of sample erro
```{r, echo=TRUE, cache=TRUE}
